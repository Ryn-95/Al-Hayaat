...
export async function generateMetadata(): Promise<Metadata> {
  const url = "https://www.pompesfunebres-alhayaat.fr/guide/doua-pour-les-parents-decedes";
  return {
...
export default function Page() {
  return (
    <div className="bg-[#F5F0E8] text-[#1A1A1A] min-h-screen font-inter">
      <SchemaOrg data={buildArticleSchema({ headline: "Doua pour les Parents Décédés en Islam", datePublished: "2024-06-15" })} />
      <section className="relative w-full pt-28 pb-16 bg-[#0D1208] text-white">
        <div className="relative z-10 w-full max-w-[1280px] mx-auto px-8 md:px-12">
          <Breadcrumbs items={[{ label: "Guide", href: "/guide" }, { label: "Doua pour les Parents Décédés en Islam", href: "/guide/doua-pour-les-parents-decedes" }]} />
          <h1 className="text-[2.2rem] md:text-[3rem] font-semibold leading-[1.1] tracking-[-0.02em] text-white mb-6 mt-6 max-w-[700px]">Doua pour les Parents Décédés en Islam</h1>
          <p className="text-[15px] font-light leading-[1.7] text-white/60 max-w-[520px]">Invocations pour les parents décédés en arabe et français.</p>
        </div>
      </section>
