...
export default function Page() { return (<div className="bg-[#F5F0E8] text-[#1A1A1A] min-h-screen font-inter"><SchemaOrg data={buildArticleSchema({ headline: "Invocations à Réciter au Cimetière en Islam", datePublished: "2024-05-01" })} /><section className="relative w-full pt-28 pb-16 bg-[#0D1208] text-white"><div className="relative z-10 w-full max-w-[1280px] mx-auto px-8 md:px-12"><Breadcrumbs items={[{ label: "Guide", href: "/guide" }, { label: "Invocations Cimetière", href: "/guide/invocations-cimetiere-islam" }]} /><h1 className="text-[2.2rem] md:text-[3rem] font-semibold leading-[1.1] tracking-[-0.02em] text-white mb-6 mt-6 max-w-[700px]">Invocations à Réciter au Cimetière (Maqbara) en Islam</h1></div></section><section className="w-full bg-[#F5F0E8] py-20 md:py-28"><div className="w-full max-w-[1280px] mx-auto px-8 md:px-12"><div className="max-w-[780px]">
  ...
  <h2 className="text-[24px] font-semibold text-[#1A1A1A] mb-6">Règles de visite au cimetière</h2>
  <ul className="flex flex-col gap-2 mb-8">{["Ne pas marcher sur les tombes", "Parler à voix basse", "Ne pas s'asseoir sur les tombes", "Réciter des douas avec sincérité"].map((t,i)=>(<li key={i} className="flex items-start gap-3 text-[14.5px] text-[#555] font-light"><span className="w-1.5 h-1.5 rounded-full bg-[#2D6A2D] mt-2 shrink-0"/>{t}</li>))}</ul>
</div></div></section></div>); }
